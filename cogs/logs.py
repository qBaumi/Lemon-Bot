...
class logs(commands.GroupCog):
    ...
    async def log(self, interaction: discord.Interaction, type: Choice[str], user: discord.User, message: str):
        ...
        print(f"INSERT INTO logs(user_id, type, msg, date, moderator_id) VALUES ('{user.id}', '{type.value}', '{message}', '{datetime.datetime.now().strftime('%Y-%m-%d')}', '{interaction.user.id}')")
