...
class economy(commands.Cog):
    ...
    async def startup(self, ctx):
        ...
        if accountopened == False:
            em = discord.Embed()
            em.add_field(name="Sorry, you already created an account!",
                         value="If you didn't read the message the first time you used this command, try: `lem help` , to get more information")
        ...
        em = discord.Embed(color=discord.Color.blurple(), title="Hello!",
                           description="Let me introduce you to our little friend Lemon right here.")
    ...
    async def balance(self, ctx):
        if await self.check_account(ctx.author) == False:
            await ctx.send("Use the `lem startup` command first!")
            return
        ...
        em.add_field(name="You have ", value=f"`{int(round(money[1], 0)):g}` golden lemons in your safe", inline=False)
    ...
    async def buy(self, ctx, item, amount=1):
        ...
        if item.lower() == "safe" and amount > 1:
            await ctx.send("You can only buy one safe!")
            return
        users = await self.get_bank_data()
        user = ctx.author
        ...
        if not res[0]:
            ...
            if res[1] == 2:
                ...
                return
            if res[1] == 5:
                await ctx.send("That item isnt in stock!")
                return
    ...
    @gift.error
    async def on_command_error(self, ctx, error):
        await ctx.send("You need to be an Admin or Mod, in order to use this command")
    ...
    async def steal(self, ctx, victim : discord.User):
        user = ctx.author
        if victim == user:
            await ctx.send("Hwat? You wanna rob yourself?!?!? I am always a thought ahead of you...trust me")
            self.steal.reset_cooldown(ctx)
            return
        if await self.check_account(ctx.author) == False:
            await ctx.send("You need to use `lem startup` first")
            self.steal.reset_cooldown(ctx)
            return
        users = await self.get_bank_data()
        if users[str(user.id)]['pocket'] < 100:
            await ctx.send("You need atleast `100 lemons` in your pocket in order to steal from another person")
            self.steal.reset_cooldown(ctx)
            return
        try:
            if users[str(victim.id)]['pocket'] < 100:
                await ctx.send("Is too `poor` to get robbed")
                self.steal.reset_cooldown(ctx)
                return
        except:
            await ctx.send("Your victim hasn't opened an account yet")
            self.steal.reset_cooldown(ctx)
            return
    ...
    async def Pay(self, ctx, userid: discord.User, pay_amount):
        ...
        if await self.check_account(userid) == False:
            await ctx.send(f"{userid} has not opened an account yet!")
            return
        if userid == ctx.author:
            await ctx.send("You cant pay yourself money...well technically, but not anymore!")
            return
        ...
        if pay_amount > 0:
            ...
            if True and pay_amount <= wallet_amt:
                await ctx.send(
                    f'You paid {userid.name} {pay_amount} lemons. If you have an emberassing image, dont forget to TAX THE HELL OUT OF THEM')
        else:
            await ctx.send('No')
    ...
    async def lottery(self, ctx, bet=0):
        if await self.check_account(ctx.author) == False:
            ...
            return
        if bet==0:
            await ctx.send("You didn't set a bet, try again `lem lottery 10` for example")
            return
        if bet<0:
            await ctx.send("You think I wouldnt see that coming? :)")
            return
        users = await self.get_bank_data()
        user = ctx.author
    ...
    async def tictactoe(self, ctx, enemy: discord.User, bet=0):
        if bet==0:
            await ctx.send(f"You didn't set a bet, try again `lem tictactoe `{enemy.name}` 10` for example")
            return
        if bet<0:
            await ctx.send("You think I wouldnt see that coming? :)")
            return
        ...
        if users[str(user.id)]['pocket'] < bet or users[str(enemy.id)]['pocket'] < bet:
            await ctx.send("You or your enemy doesn't have enough money in their pocket!")
            return
        ...
        try:
            ...
            if reaction.emoji == "❌":
                await ctx.send(f"{enemy.name} dosent wanna play tictactoe with you right now <:Sadge:720250426892615745>")
                return
    ...
    async def wyr(self, ctx):
        ...
        await ctx.send("You succesfully paid your 25 lemons for this question!")
        ...
        await ctx.send(f"Would you rather {scene} or {scene2}")
    ...
    async def use(self, ctx, item="None"):
        if await self.check_account(ctx.author) == False:
            ...
            return
        if item == "None":
            await ctx.send("You cant use nothing")
            return
        user = ctx.author
        users = await self.get_bank_data()
        checkifitem = 0
        ...
        for item_ in users[str(user.id)]["bag"]:
            ...
            if item.lower() == item_name.lower():
                if item_amount <= 0:
                    await ctx.send(f"You dont have {item_name.capitalize()}")
                    return
                else:
                    checkifitem = 1
                break
        if checkifitem == 0:
            ...
            if users[str(user.id)]["bag"][index]["item"] != item.lower():
                await ctx.send(f"You dont have {item.capitalize()}")
                return
        item = item.lower()
        if item == "lemonade":
            await ctx.send("You just drank lemonade that was made by lemons, that you bought with the lemons, that you get paid as a lemon farmer for harvesting lemons")
        ...
        if item == "flowers":
            await ctx.send("Who will be the happy one that gets your beautiful flowers?")
            def check(m):
            ...
            try:
                ...
                try:
                    ...
                    await ctx.send("Self defending mechanism activated. Something didnt work, qBaumi doesnt know why, but if anyone lost something CONTACT him. RIGHT NOW")
                    return

            except:
                await ctx.send(f"{msg.content} is not a user or has never used this bot before. `Answer with @friend if you just typed their name`")
                return

        if item == "safe":

            def check(m):
                return m.author == ctx.author and m.channel == ctx.channel and m.content.lower() == "dep" or m.author == ctx.author and m.channel == ctx.channel and m.content.lower() == "depot" or m.author == ctx.author and m.channel == ctx.channel and m.content.lower() == "with" or m.author == ctx.author and m.channel == ctx.channel and m.content.lower() == "withdraw" or m.author == ctx.author and m.channel == ctx.channel and m.content.lower() == "witd"
            ...
            await ctx.send(f"{user.mention}\nDo you want to `depot` or `withdraw` money from your safe?", embed=em)
            try:
                msg = await self.client.wait_for("message", timeout=60, check=check)
            except:
            ...
            if msg.content.lower() == "dep" or msg.content.lower() == "depot":
                userbal = users[str(user.id)]["pocket"]

                def checkmoney(m):
                    return m.author == ctx.author and m.channel == ctx.channel
                await ctx.send(f"{user.mention}\nHow much lemons do you want to depot?")
                try:
            ...
            if msg.content.lower() == "withdraw" or msg.content.lower() == "witd" or msg.content.lower() == "with":
                ...
                try:
                    ...
                    return
                if amountmoney < 0:
                    await ctx.send(f"{user.mention}\nYou know, you can also `depot` money")
                    return
        ...
        if item == "present":
            await ctx.send("What item do you want to gift?")
            ...
            for present_item in users[str(user.id)]["bag"]:
                ...
                if msg.content.lower() == present_item_name.lower():
                    if present_item_amount <= 0:
                        await ctx.send(f"You dont have {present_item_name.capitalize()}")
                        return
                    itemreal = 1
                    break
            if itemreal == 0:
                await ctx.send("That item doesnt exist or you dont have it!")
                return
            ...
            await ctx.send("Who will be the happy one? (@ them)")
            ...
            try:
                ...
                print(id)
                if user.id == id.id:
                    await ctx.send("You cant just give yourself a present!")
                    return

                users[str(user.id)]["bag"][index]["amount"] = item_amount - 1

                await ctx.send(f"You gifted {present_item_name} to {msg.content}, they " + line)
                with open('lemonbank.json', 'w') as f:
                    json.dump(users, f, indent=4)
        ...
        if item == "conchshell":
            await ctx.send("Ahhh I see, you need Trustpilot 10⭐ advice...what lies on your heart my friendo?")
            def check(m):
                return m.author == ctx.author and m.channel == ctx.channel
            try:
                msg = await self.client.wait_for("message", timeout=60, check=check)
        ...
        if item == "laptop":
            ...
            if str(reaction.emoji) == "<:GoogleChrome:883281638270844958>":
                msg = await ctx.send("What do you want to browse on the web?\n`animals`\n`memes`\n`random facts`")
                try:
                    msg = await self.client.wait_for("message", timeout=60, check=check)
                except:
                    await ctx.send("You didnt answer in time")
        ...
        if item == "cheesecake":
            ...
            def checkperson(m):
                return m.author == ctx.author and m.channel == ctx.channel
            await ctx.send("Do you want to `eat`, `throw` or `share` the cake? (Answer with `eat`, `throw` or `share`)")
            ...
            if msg.content.lower() == "eat":
                await ctx.send("You ate your cheesecake, it was very delicious")
                users[str(user.id)]["bag"][index]["amount"] = item_amount - 1
            elif msg.content.lower() == "share":
                await ctx.send("With whom you want to share your cheesecake?")
                try:
                    ...
                    return
                person = msg.content
                await ctx.send(f"You shared your cake with {person}!")
                users[str(user.id)]["bag"][index]["amount"] = item_amount - 1
            else:
                await ctx.send("Who will be your victim?")
                try:
                    msg = await self.client.wait_for("message", timeout=60, check=checkperson)
                except:
                    await ctx.send("You didnt answer in time")
                    return
                person = msg.content
                await ctx.send(f"You throw your cake at {person}")
                users[str(user.id)]["bag"][index]["amount"] = item_amount - 1
            with open('lemonbank.json', 'w') as f:
                json.dump(users, f, indent=4)
            return
        ...
        if item == "pinata":
            ...
            await ctx.send("Do you want to `use` or `gift` the pinata? (Answer with `use` or `gift`)")
            ...
            if msg.content.lower() == "use":
                ...
                print(candy)
                await ctx.send(f"After you hit 🏏 the pinata `{times}` times you got {candy} candy!")
                users[str(user.id)]["bag"][index]["amount"] = item_amount - 1
                with open('lemonbank.json', 'w') as f:
                    json.dump(users, f, indent=4)
                ...
                await ctx.send("Who gets the pinata?")
                try:
                    msg = await self.client.wait_for("message", timeout=60, check=checkperson)
                except:
                    await ctx.send("You didnt answer in time")
                ...
                try:
                    ...
                    print(id)
                    users[str(user.id)]["bag"][index]["amount"] = item_amount - 1
                    await ctx.send(f"You gifted your pinata to {msg.content}, muchas gracias they said")
                    with open('lemonbank.json', 'w') as f:
                        json.dump(users, f, indent=4)
                    try:
                    ...
                    await ctx.send(
                        f"{msg.content} is not a user or has never used this bot before. `Answer with @friend if you just typed their name`")
                    return
            ...
            await ctx.send("That item does not exist or has no usage yet")
    ...
    async def work(self, ctx):
        ...
        for job in userjob:
            ...
            if name == 'tiktokhater':
                ...
                print(reaction.emoji)
                if reaction.emoji == '🔇':
                    line = "You muted him for 24h"
                elif reaction.emoji == '⛔':
                    line = "You kicked him from the server"
                elif reaction.emoji == '🚫':
                    line = "You banned him from the server"
                else:
                    line = "Idk"
                ...
                if rndmaufgabe == 1:
                    ...
                    try:
                        ...
                        await ctx.send('You didnt answer in time and the lemons were eaten by some birds')
                        return
                    ...
                    try:
                        ...
                        await ctx.send('You didnt answer in time and your lemon trees sadly died :(')
                        return
                ...
                await ctx.send('I think the creator didnt finish this job yet...')
                self.work.reset_cooldown(ctx)
